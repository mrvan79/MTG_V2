...
public class OperationController {
	...
	public String postOperationDelete(TableModel tableModel) {
		try {
			...
			if(tableModel.getTableName().equals("purpose")) {
				fileLogToSQL.purposeHistoryEdit(operationService.selectPurposeHistory(tableModel.getId()));
			}else {
				fileLogToSQL.chargeHistoryEdit(operationService.selectChargeHistory(tableModel.getId()));
			}
	...
	public String postOperationReg(TableModel tableModel,Model model) {
		try {
			//tableNameとして新規登録
			if(tableModel.getTableName().equals("user_infor") && operationService.checkUserNameAndEmail(tableModel).size()==0) {
				...
				model.addAttribute("user", tableModel.getUsername());
				model.addAttribute("email", tableModel.getEmail());
				model.addAttribute("pass", pasString);
				fileLogToSQL.chargeHistory(operationService.selectChargeHistory(tableModel.getId()),pasString);
				return "operation/regUser";
	...
	public String resetPassUser(TableModel tableModel,Model model) {
		String pasString = coverRegister.createPassword();
		tableModel.setPassword(encoder.encode(pasString));
		System.out.println(tableModel);
		operationService.updatePassword(tableModel);
		TableModel historyModel = operationService.selectChargeHistory(tableModel.getId());
		model.addAttribute("user", historyModel.getUsername());
		model.addAttribute("email", historyModel.getEmail());
		model.addAttribute("pass", pasString);
		fileLogToSQL.chargeHistory(historyModel,pasString);
